...
public:
    ...
    shared_ptr<vector<pq>> InitPriorityQueue();
    ...
    void Insert(shared_ptr<vector<pq>> queue, string text, float priority);
    ...
    string Remove(shared_ptr<vector<pq>> queue);
    ...
    // should return the empty string.
    string Peek(shared_ptr<vector<pq>> queue);    

private:
    // you can add add more private member variables and member functions here if you need
    void heapifyUp(shared_ptr<vector<pq>> queue, int index);
    void heapifyDown(shared_ptr<vector<pq>> queue, int index);
};
